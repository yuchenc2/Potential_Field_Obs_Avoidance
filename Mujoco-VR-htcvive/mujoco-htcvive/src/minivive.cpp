...
void v_copyPose(const TrackedDevicePose_t* pose, float* roompos, float* roommat)
{
    ...
    // raw data: room
    roompos[0] = p->m[0][3]; //add robot location to attached to the robot Johnny
    roompos[1] = p->m[1][3]; //add robot location to attached to the robot
    roompos[2] = p->m[2][3]; //add robot location to attached to the robot
    roommat[0] = p->m[0][0];
    roommat[1] = p->m[0][1];
    roommat[2] = p->m[0][2];
    roommat[3] = p->m[1][0];
